...
class rgb_net(nn.Module):
    def __init__(self, filters=32):
        super().__init__()
        self.rgb = resnet50(weights=None)
    ...
    def forward(self, rgb):
        ...
        dec5 = self.dec5(rgb4)
        dec5 = nn.functional.max_pool2d(dec5, kernel_size=(2, 2), stride=2)
...
class nnn_net(nn.Module):
    def __init__(self, filters=32):
        super().__init__()
        self.nnn = resnet50(weights=None)
...
class MSNet(nn.Module):
    ...
    def __init__(self, n_classes, n_channels=4):
        ...
        self.fuse = nn.Conv2d(32, n_classes, kernel_size=3, padding=1)

    def forward(self, rgbn):
        input_size = (rgbn.size()[2], rgbn.size()[3])
        # RGB, (NIR or NDWI) (rgbn) are stacked together
        # # of required channels in MSNet? -> 6 channels (RGB, NIR, NIR, NIR) -> NIR (1 channel)
        rgb = rgbn[:, :3, :, :]
        nir = rgbn[:, 3:, :, :]
        if nir.shape[1] == 1:
            nnn = nir.repeat(1, 3, 1, 1)
        else:
            nnn = rgbn[:, 3:, :, :]

        # Extract features (checked _ run correctly)
